...
def setup_module():
    ...
    dir_bucket_params = {
        ...
        'CreateBucketConfiguration': {
            'Location': {'Type': 'AvailabilityZone', 'Name': _DEFAULT_AZ},
            'Bucket': {
                'Type': 'Directory',
...
class TestMoveCommand(BaseS3IntegrationTest):
    ...
    def test_mv_s3_to_s3_multipart_recursive(self):
        ...
        self.assertTrue(self.key_not_exists(from_bucket, key_name='largefile'))
        self.assertTrue(self.key_not_exists(from_bucket, key_name='smallfile'))
    ...
    def test_mv_with_large_file(self):
        ...
        self.assertEqual(
            os.path.getsize(foo_txt), len(file_contents.getvalue())
        )
    ...
    def test_cant_move_large_file_onto_itself(self):
        ...
        bucket_name = _SHARED_BUCKET
        self.put_object(
            bucket_name, key_name='key.txt', contents=file_contents
        )
        p = aws(
            's3 mv s3://%s/key.txt s3://%s/key.txt'
            % (bucket_name, bucket_name)
...
class TestCp(BaseS3IntegrationTest):
    ...
    def test_download_large_file(self):
        ...
        self.assert_no_errors(p)
        self.assertEqual(
            os.path.getsize(local_foo_txt), len(foo_contents.getvalue())
        )
    ...
    def test_download_ctrl_c_does_not_hang(self):
        ...
        process = aws(
            's3 cp s3://%s/foo.txt %s --quiet' % (bucket_name, local_foo_txt),
    ...
    def test_cleans_up_aborted_uploads(self):
        ...
        # to the PIPE, causing a deadlock when not consumed.
        process = aws(
            's3 cp %s s3://%s/ --quiet' % (foo_txt, bucket_name),
            wait_for_finish=False,
        )
        time.sleep(3)
        # The process has 60 seconds to finish after being sent a Ctrl+C,
    ...
    def test_download_empty_object(self):
        ...
        p = aws(
            's3 cp s3://%s/%s %s' % (bucket_name, object_name, local_filename)
        )
        self.assertEqual(p.rc, 0)
    ...
    def test_copy_metadata_directive(self):
        ...
        self.put_object(
            bucket_name, original_key, contents='foo', extra_args=metadata
        )
        p = aws(
            's3 cp s3://%s/%s s3://%s/%s'
    ...
    def test_cp_with_request_payer(self):
        ...
        foo_txt = self.files.create_file('foo.txt', 'this is foo.txt')
        p = aws(
            's3 cp %s s3://%s/mykey --request-payer' % (foo_txt, bucket_name)
        )
...
class TestSync(BaseS3IntegrationTest):
        ...
        for i in range(4):
            ...
            filenames.append(
                self.files.create_file(
                    'foo +%06d' % i, contents='', mtime=mtime
                )
            )
        p = aws(
    ...
    def test_sync_delete_locally(self):
        ...
        p = aws(
            's3 sync s3://%s/ %s --delete' % (bucket_name, self.files.rootdir)
        )
        self.assert_no_errors(p)
        ...
        self.assertTrue(
            os.path.exists(os.path.join(self.files.rootdir, 'bar.txt'))
        )
...
class TestWarnings(BaseS3IntegrationTest):
    def test_no_exist(self):
        ...
        self.assertIn(
            'The user-provided path %s does not exist.' % filename, p.stderr
        )
...
class TestUnableToWriteToFile(BaseS3IntegrationTest):
    ...
    def test_no_write_access_large_file(self):
        ...
        self.put_object(bucket_name, 'foo.txt', contents=contents)
        os.chmod(self.files.rootdir, 0o444)
        p = aws(
            's3 cp s3://%s/foo.txt %s'
            % (bucket_name, os.path.join(self.files.rootdir, 'foo.txt'))
...
class TestSymlinks(BaseS3IntegrationTest):
    ...
    def extra_setup(self):
        ...
        self.sample_file = self.files.create_file(
            os.path.join(self.nested_dir, 'foo.txt'),
            contents='foo.txt contents',
        )
        # Create a symlink to foo.txt.
        os.symlink(
            self.sample_file, os.path.join(self.files.rootdir, 'a-goodsymlink')
        )
        ...
        os.symlink(
            self.nested_dir, os.path.join(self.files.rootdir, 'c-goodsymlink')
        )
    ...
    def test_no_follow_symlinks(self):
        ...
        self.assert_no_errors(p)
        self.assertTrue(self.key_not_exists(self.bucket_name, 'a-goodsymlink'))
        self.assertTrue(self.key_not_exists(self.bucket_name, 'b-badsymlink'))
        self.assertTrue(
            self.key_not_exists(self.bucket_name, 'c-goodsymlink/foo.txt')
        )
        self.assertEqual(
            self.get_key_contents(
                self.bucket_name, key_name='realfiles/foo.txt'
            ),
            'foo.txt contents',
        )
    ...
    def test_follow_symlinks(self):
        ...
        self.assert_no_errors(p)
        self.assertEqual(
            self.get_key_contents(self.bucket_name, key_name='a-goodsymlink'),
            'foo.txt contents',
        )
        self.assertTrue(self.key_not_exists(self.bucket_name, 'b-badsymlink'))
        self.assertEqual(
            self.get_key_contents(
                self.bucket_name, key_name='c-goodsymlink/foo.txt'
            ),
            'foo.txt contents',
        )
        self.assertEqual(
...
class TestUnicode(BaseS3IntegrationTest):
    ...
    def test_cp(self):
        bucket_name = _SHARED_BUCKET
        local_example1_txt = self.files.create_file(
        ...
        local_example2_txt = self.files.full_path('\u00e9xample2.txt')
    ...
    def test_recursive_cp(self):
        ...
        local_example2_txt = self.files.create_file(
...
class TestOutput(BaseS3IntegrationTest):
    ...
    def test_normal_output_only_show_errors(self):
        ...
        # Copy file into bucket.
        p = aws(
            's3 cp %s s3://%s/ --only-show-errors' % (foo_txt, bucket_name)
        )
        self.assertEqual(p.rc, 0)
        # Check that nothing was printed to stdout.
        self.assertEqual('', p.stdout)
    ...
    def test_error_ouput_only_show_errors(self):
        ...
        # Copy file into bucket.
        p = aws(
            's3 cp %s s3://non-existant-bucket/ --only-show-errors' % foo_txt
        )
        # Check that there were errors and that the error was print to stderr.
        self.assertEqual(p.rc, 1)
        self.assertIn('upload failed', p.stderr)
...
class TestIncludeExcludeFilters(BaseS3IntegrationTest):
    ...
    def test_recursive_exclude(self):
        ...
        p = aws(
            ...
            "--exclude '*' --include 'test/*' --recursive" % self.files.rootdir
        )
        self.assert_no_errors(p)
    ...
    def test_s3_filtering(self):
        ...
        p = aws(
            "s3 rm s3://%s/ --dryrun --exclude '*' --recursive" % bucket_name
        )
        self.assert_no_files_would_be_uploaded(p)
    ...
    def test_exclude_filter_with_relative_path(self):
        ...
        try:
            ...
            p = aws(
                "s3 sync . s3://%s/ --exclude '*.py' --delete" % bucket_name
            )
        finally:
            os.chdir(cwd)
        self.assert_no_errors(p)
    ...
    def test_filter_no_resync(self):
        ...
        self.files.create_file(
            os.path.join(dir_name, 'test.txt'), contents='foo'
        )
...
class TestFileWithSpaces(BaseS3IntegrationTest):
    def test_upload_download_file_with_spaces(self):
        ...
        p = aws(
            's3 cp %s s3://%s/ --recursive' % (self.files.rootdir, bucket_name)
        )
        self.assert_no_errors(p)
        os.remove(filename)
        # Now download the file back down locally.
        p = aws(
            's3 cp s3://%s/ %s --recursive' % (bucket_name, self.files.rootdir)
        )
        self.assert_no_errors(p)
...
class TestStreams(BaseS3IntegrationTest):
    ...
    def test_unicode_upload(self):
        ...
        self.assertTrue(self.key_exists(bucket_name, 'stream'))
        self.assertEqual(
            self.get_key_contents(bucket_name, 'stream'), unicode_str
        )
    ...
    def test_multipart_upload(self):
        ...
        p = aws(
            's3 cp - s3://%s/stream' % bucket_name, input_data=data_encoded
        )
        self.assert_no_errors(p)
        self.assertTrue(self.key_exists(bucket_name, 'stream'))
...
class TestSSERelatedParams(BaseS3IntegrationTest):
    ...
    def test_smoke_sync_sse(self):
        ...
        # Copy sync
        p = aws(
            's3 sync s3://%s/foo/ s3://%s/bar/ --sse AES256' % (bucket, bucket)
        )
        self.assert_no_errors(p)
